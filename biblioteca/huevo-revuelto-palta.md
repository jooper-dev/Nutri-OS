---
id: huevo-revuelto-palta
titulo: Revuelto con palta
subtitulo:
edad_min_meses: 6
porciones: 1
unidades_por_porcion: null
medida_porcion: ½ taza
tiempo_min: 8
dificultad: Muy fácil
momento: [desayuno]
componente: acompanante
familia: huevo
textura: humeda
aporta: [proteina, grasas_saludables]
alergenos_presentes: [huevo]
etiquetas: [sin-gluten]
conservacion:
  ambiente_dias: 0
  refri_dias: 1
  congelador_meses: 0
origen: creada
validada_en_cocina: false
variante_foto: D
props_foto:
acento: "#F2E3B3"
---

# Revuelto con palta

+6 m · 1 porc. (½ taza c/u) · 8 min · Muy fácil

sin gluten

## Nota de la Nutricionista

Cremoso, suave y de sabor amable: suele ser de los primeros platos que un bebé acepta sin negociar. La palta le da la densidad que a esta edad hace falta en poco volumen.

## Ingredientes

• **1** huevo mediano
• **2 cdas** palta madura · 30 g
• **1 cdta** aceite de oliva · 5 ml

## Preparación

01  **Bate**. el huevo hasta integrar bien la yema.
02  **Cocina**. a fuego muy bajo removiendo, hasta que cuaje sin dorarse.
03  **Aplasta**. la palta e intégrala fuera del fuego.

## Ideas

• Sin huevo → usa **3 cdas** de puré de menestra con palta.
• Sin palta → usa **1 cdta** de aceite de oliva extra.

## Conservación

Dura: 1 días refri

## Foto

Huevo revuelto de textura cremosa y grumo suave, color amarillo mantequilla, formando un montículo bajo de unos 8 cm de ancho, con vetas verdes de palta aplastada integradas de forma irregular. Superficie mate y húmeda, sin dorado ni bordes secos.

--- NOTA PARA PATY ---

RECETA CREADA — sin probar en cocina. Receta semilla, generada para poner en marcha el motor de Nutri-OS. Valídala o reemplázala antes de usarla con una paciente real.

Correcciones: sin correcciones (no hubo fuente que auditar).
Etiquetas asignadas: derivadas de la lista final de ingredientes, incluidas las opciones.
Datos asumidos: conservación y rendimiento con valores estándar.
Acento de color: #F2E3B3.
