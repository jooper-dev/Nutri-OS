...
- **EJECUCIÓN Y FORMATO DE SALIDA OBLIGATORIO (ESTRUCTURA JSON DESGLOSADA):**
  ...
  **REGLAS DE DESGLOSE:** Cada comida debe estar dividida en sus sub-componentes exactos. Si un sub-componente no aplica para ese día (ej. no hay menestra, o no hay página de receta), debes dejar el valor estrictamente como un string vacío `""`. Si hay página, colócala SÓLO en su llave correspondiente.
